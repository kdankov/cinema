	...
	request(url, function(error, response, html){
		if(!error){
			...
			$('td.featureName').each(function(i, elem){
				...
				var movietrailer_url = 'https://www.googleapis.com/youtube/v3/search?key=AIzaSyByrIAe8jAqD4DtHuLCe_6LZcVDjHfewtY&part=snippet&q=' + movieName.replace(' ', '+') + ' trailer&maxResults=1&type=video&videoDefinition=high&videoEmbeddable=true';
				...
				request(movietrailer_url, function(t_error, t_response, t_html){
					if(!error){
						...
						if ( movietrailer_json['items'][0]['id']['videoId'] != undefined ) {

							var trailerLink = movietrailer_json['items'][0]['id']['videoId'];
