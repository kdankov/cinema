...
function parseCinemaCity($url, $json, $html){
  ...
  foreach( $scraper->find('td[class="featureName"]') as $entry) {
    ...
    $movietrailer_url = 'http://gdata.youtube.com/feeds/api/videos?q='. str_replace(' ', '+', $movieName) .'-trailer&start-index=1&max-results=1&v=2&alt=json&hd';
    $movietrailer_json = file_get_contents($movietrailer_url);
    $movietrailer_data = json_decode($movietrailer_json, TRUE);
    $movietrailer_data = explode("v=", $movietrailer_data["feed"]["entry"][0]["link"][0]["href"]);
    $movietrailer_data = explode("&", $movietrailer_data[1]);
    $movietrailer_data = $movietrailer_data[0];
    $movietrailer_link = 'http://www.youtube.com/watch?v='.$movietrailer_data;
